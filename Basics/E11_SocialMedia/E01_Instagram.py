...
def draw(contextName):
    ...
    for bs2, bs3, bs4, imagePath, alt in bannerData:
        ...
        if bs2 is not None:
            ...
            newRect(x=pad, y=h-th-2*tPad, fill=color(rgb='red', a=0.8),
                    w=w-2*pad, h=th+2*tPad, parent=page)
            newText(bs2, x=pad+tPad, y=h-2*tPad, w=w-2*pad-2*tPad, parent=page)

        if bs3 is not None:
            tw, th = context.textSize(bs3, w=w-2*pad-2*tPad)
            newRect(x=pad, y=h/2-3.5*tPad, fill=color(rgb='darkblue', a=0.8),
                    w=w-2*pad, h=th+2*tPad, parent=page)
            newText(bs3, x=pad+tPad, y=h/2, w=w-2*pad-2*tPad, parent=page)

        if bs4 is not None:
            tw, th = context.textSize(bs4, w=w-2*pad-2*tPad)
            newRect(x=pad, y=0, fill=color(rgb='blue', a=0.8), w=w-2*pad,
                    h=th+2*tPad, parent=page)
